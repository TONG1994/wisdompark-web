...
module.exports = {
    // 获取选中的园区uuid
    getSelectedParkUuid:function () {
        let selectedParkUuid = window.sessionStorage.selectedParkUuid||window.selectedParkUuid;
        if(!selectedParkUuid) {
           return '127V0A3L79AVP001';
        }
        return selectedParkUuid;
      },
      getParkList:function () {
        let selectParkData = window.sessionStorage.selectParkData||window.selectParkData;
        if(!selectParkData) {
          return null;
        }
        let selectParkDataObj;
        if(typeof selectParkData === 'string'){
          try {
            selectParkDataObj = JSON.parse(selectParkData);
          }catch (err){}
        }else{
          selectParkDataObj = selectParkData;
        }
        if(selectParkDataObj){
            return selectParkDataObj;
        }
      },
    //获取登录信息
  ...
  //获取登录者的类型
    ...
    initTreeNodes3: function (dataSet, funcCrtGroupNode, funcCrtNode) {
        ...
        var otherNode = null;
        dataSet.map((data, i) => {
            var group = funcCrtGroupNode(data, dataSet);
            if (group.isOther === true) {
                otherNode = group;
            }
            else {
                var node = groupMap[group.key];
                if (!node) {
        ...
        if (otherNode) {
            groupMap[otherNode.key] = otherNode;
            otherNode.children = [];
            pNodes.push(otherNode);
        }
    ...
    getRecordSet: function (recordSet, pageSize) {
        if (!recordSet) {
            return [];
        }

        if (recordSet.length <= pageSize) {
            return recordSet;
        }

        var ret = [];
        var startRow = recordSet.length - pageSize;
        for (var i = 0; i < pageSize; i++) {
            ret.push(recordSet[i + startRow]);
        }
...
var _isValidMethod = [
    ...
    {
        type: 'passwd',
        isValid: function (str) { if (FormVerify.regPsw.test(str) == false) return '请输入6~16位字母、数字二者组合!'; }
    }
];
