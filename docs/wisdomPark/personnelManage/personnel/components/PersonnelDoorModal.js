...
let PersonnelDoorModal = React.createClass({
  getInitialState: function () {
    return {
      loading: false,
      modal: false,
      settledPersonnelDoor: {},
      hints: {},
      validRules: [],
      park: [],
      building: [],
  ...
  mixins: [Reflux.listenTo(PersonnelStore, 'onServiceComplete'), ModalForm('settledPersonnelDoor')],
  onServiceComplete: function (data) {
    if (data.errMsg) {
      this.setState({ errMsg: data.errMsg });
      return;
    }
    if (this.state.modal && data.operation === 'getSettledPersonnelSelectDoor') {
      //0园区，1楼宇，2楼层，3单元，4房间
      ...
      dataArray.map(item => {
        ...
        let door = item.doorName.substring(0,20) + '-' + property;
  ...
  initEditDatas: function (settledPersonnelDoor) {
    // let dataUrl = 'http://gridfs-cs.icerno.com/gridfs/find?fileId=' + settledPersonnelDoor.picture;
    let dataUrl = Utils.fileAddress + 'gridfs/find?fileId=' + settledPersonnelDoor.picture ;
    this.setState({ dataUrl });
  },

  //门禁权限
  initEditData: function (settledPersonnelDoor) {

    this.state.hints = {};
    Utils.copyValue(settledPersonnelDoor, this.state.settledPersonnelDoor);
    this.setState({
      loading: false,
    });
    ...
    //PersonnelActions.getSettledPersonnelSelectedDoor({"object":{ uuid: '12530L1AAMO0892F' }})
    PersonnelActions.getSettledPersonnelSelectedDoor({ "object": { uuid: settledPersonnelDoor.uuid } })
  },

  clear: function () {
    FormDef.initSettledCompanyForm(this.state.settledPersonnelDoor);
    // FIXME 输入参数，对象初始化
    this.state.hints = {};
  ...
  onClickSave: function () {
    //if (Common.formValidator(this, this.state.settledPersonnelDoor)) {
    //  this.setState({ loading: true });
    //  settledPersonnelDoorActions.addUser(this.state.settledPersonnelDoor);
    //}
  },

  render: function () {
    let title = this.props.doorActionType === 'check' ? '查看门禁权限' : this.props.doorActionType === 'edit' ? '编辑门禁权限' : this.props.doorActionType === 'showPicture' ? '查看身份图片' : '模态框';
    return (
      <Modal
      ...
      >
        {
          this.props.doorActionType == 'showPicture' ? <img style={{ display: 'block', maxWidth: '100%',height: '300px', margin: '0 auto' }} src={this.state.dataUrl} /> :
            this.state.park.length == 0 && this.state.building == 0 && this.state.floor == 0 ? <span>未设置门禁权限</span> :
...
export default PersonnelDoorModal;
